# Approval rules for a vault import

A human accepts a vault import plan. The server applies only the accepted plan.
These rules protect that boundary. Follow every rule.

## Plan and apply are separate steps

- Create the plan with `artifactbridge_plan_document_import`. This step does
...
- The selected note set is part of the plan. When the human changes the
  selection query, the plan changes.
...
- Do not accept the proposals for the human. This skill stays pinned to the
  accepted versions.
- Do not bypass the document proposal gate.
